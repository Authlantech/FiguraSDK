...
namespace fgr
{
	...
	public : 	
		...
		void									init_engine(const char* title,int width,int height);
		void									update_window();
		bool									window_is_open();

		std::shared_ptr<fgr::Model>				create_model(std::string name);
		std::shared_ptr<fgr::Model>				create_and_load_model(std::string name, std::string path);
